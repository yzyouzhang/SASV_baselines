...
class train_dataset_sampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        mixed_list = []
        num_iter = len(mixed_lists['bonafide']) // self.batch_size * 4
        for i in range(num_iter):
            bna_samples = mixed_lists['bonafide'][i*self.batch_size//4: (i+1)*self.batch_size//4]
            spf_samples = mixed_lists['spoof'][i*self.batch_size//4: (i+1)*self.batch_size//4]
            if len(bna_samples) != len(spf_samples):
                break
            mixed_list.extend(bna_samples+spf_samples)
